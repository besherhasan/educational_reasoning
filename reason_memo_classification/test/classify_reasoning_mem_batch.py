...
# -*- coding: utf-8 -*-
"""
End-to-end script: classify every MMLU (English) question into Reasoning vs Memorization
...
def load_mmlu_all() -> Dict[str, Any]:
    ...
    Splits vary by release; we iterate over whatever is present.
...
def build_messages(question_text: str, options: List[str]) -> List[Dict[str, Any]]:
    ...
    + "\n\nOutput JSON only with keys Reasoning and Memorization."
    )
...
def call_model_with_backoff(
    ...
    while True:
        try:
            ...
            kwargs["max_completion_tokens"] = 20   # correct param for SDK v1.x
   # cap output to ~20 tokens
