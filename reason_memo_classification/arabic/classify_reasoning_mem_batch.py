...
# -*- coding: utf-8 -*-
"""
End-to-end script: classify every Arabic MMLU question into Reasoning vs Memorization
with soft percentages using OpenAI's Batch API.

Outputs
- JSONL:  arabicmmlu_reasoning_labels.jsonl  (written per row on result ingest)
- XLSX:   arabicmmlu_reasoning_labels.xlsx   (appended per row on result ingest)
- Checkpoint: reasoning_checkpoint.json      (updated on every ingest or poll)
- Optional snapshots: arabicmmlu_reasoning_labels.json (valid JSON array)

Usage
  export OPENAI_API_KEY=sk-...
  python classify_arabicmmlu_reasoning_batch.py \
      --model gpt-5 \
      --outdir ./out \
...
def build_messages(question_text: str, options: List[str]) -> List[Dict[str, Any]]:
    # System prompt tailored to 2-class reasoning vs memorization
    sys_msg = (
        "You are an expert educational assessor. "
        "Classify the cognitive demand to answer a question into two categories with integer percentages that sum to 100: "
        "Reasoning and Memorization. "
        "Reasoning covers inference, multi-step thinking, comparison, analysis, synthesis, evaluation, or applying rules to novel cases. "
        "Memorization covers direct recall of facts, definitions, formulas, dates, or simple one-step retrieval. "
        "Return ONLY a JSON object with exactly these keys: Reasoning, Memorization. "
        "Values must be integers in [0,100] and sum to 100. No extra text."
    )
    parts = [f"Question:\n{question_text.strip()}"]
    if options:
...
def normalize_distribution(d: Dict[str, Any]) -> Dict[str, int]:
    # Convert any numeric-like inputs to floats; clamp negatives; renormalize to sum 100, then round.
    vals = []
    ...
    total = sum(vals)
    if total <= 0:
        # Fallback: default to Memorization-heavy if model failed
...
def build_batch_tasks(
    ...
    for ukey, row in pending_rows:
        ...
        task = {
            "custom_id": ukey,               # used to join results back
            "method": "POST",
            "url": "/v1/chat/completions",   # same params as chat.completions
            "body": body
        }
        tasks.append(task)
    return tasks
...
def main():
    ...
    jsonl_path = outdir / "arabicmmlu_reasoning_labels.jsonl"
    json_snapshot_path = outdir / "arabicmmlu_reasoning_labels.json"
    xlsx_path = outdir / "arabicmmlu_reasoning_labels.xlsx"
    ckpt_path = outdir / "reasoning_checkpoint.json"
    ...
    if args.debug:
        ...
        for ukey, row in debug_rows:
            messages = build_messages(row["Question"], [row.get(f"Option {i}") for i in range(1, 6)])
    ...
    for bi, chunk_rows in enumerate(chunks, start=1):
        ...
        if output_file_id:
            ...
            ingested = 0
            # Build a quick source map for this chunk for O(1) lookups
